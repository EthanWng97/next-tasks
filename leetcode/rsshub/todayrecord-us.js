...
module.exports = async (ctx) => {
    ...
    let responseBasic;
    const recordPayload =
        '{"query":"    query questionOfToday {  currentDailyCodingChallenge {    questionOfToday {      date      userStatus      link      question {        acRate        difficulty        freqBar        frontendQuestionId: questionFrontendId        isFavor        paidOnly: isPaidOnly        status        title        titleSlug        hasVideoSolution        hasSolution        topicTags {          name          id          slug        }      }    }  }}    ","variables":{}}';
    ...
    let data = responseBasic.data.data.currentDailyCodingChallenge.questionOfToday;
    let date = data.date;
