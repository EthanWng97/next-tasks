...
GitHub: NavePnow
*/
const accounts = [["username@xxx.com", "xxx", "x", "xxxx"]];
    ...
    if (error) {
      ...
      if (data.match(/(欢迎您回来)/)) {
        ...
        if (formhash_login) {
          ...
          if (formhash_login1) {
            formhash_login1 = formhash_login1[0].replace(
              "formhash\" value='",
              ""
            );
            await login(
...
function login(username, password, questionid, answer, formhash) {
  let table1 = {
    ...
    body:
      "formhash=" +
      formhash +
      "&referer=https%3A%2F%2Fwww.1point3acres.com%2Fbbs%2F.%2F&fastloginfield=username&username=" +
      username +
      "&password=" +
      password +
      "&submit=%E7%99%BB%E5%BD%95&questionid=" +
      questionid +
      "&answer=" +
      answer +
      "&cookietime=2592000",
